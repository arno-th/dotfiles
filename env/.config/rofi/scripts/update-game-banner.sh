...
update-banner() {
    ...
    local src=$(find ${STEAM_ROOT}/appcache/librarycache/${appid} -type f -name 'library_hero*' | head -n 1)
    local title=$(find ${STEAM_ROOT}/appcache/librarycache/${appid} -type f -name 'logo*' | head -n 1)
    local out=$CACHE_DIR/$appid/${width}x${height}.$EXT
    echo "Hero path: $src"
    echo "Logo path: $title"
    echo "Out path: $out"
